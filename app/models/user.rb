...
class User < ApplicationRecord
  ...
  def correct_image_mime_type
    return unless profile_picture.attached? && !profile_picture.content_type.in?(%w[image/png image/jpg image/jpeg])

    errors.add(:profile_picture, 'はPNGまたはJPEG画像でなければなりません')
  end

  def correct_image_size
    return unless profile_picture.attached? && profile_picture.byte_size > 5.megabytes
